...
class Regression(object):
    ...
    def construct_polynomial_feats(
        ...
        if x.ndim == 1:
            feat = np.hstack(feat).reshape(N, degree + 1)
    ...
    def linear_fit_GD(
        ...
        weight = np.zeros(xtrain[0].shape, 1)
        loss_per_epoch = np.array(epochs, )

        for i in range (epochs):
            weight += (learning_rate / xtrain.shape) * np.transpose(xtrain).np.matmul(ytrain - (xtrain * weight))
            loss_per_epoch[i] = self.rmse(weight * xtrain, ytrain)
    ...
    def linear_fit_SGD(
        ...
        for i in range (epochs):
            for j in range (xtrain.shape):
                error = np.dot(xtrain[i], weight) - ytrain[i]
    ...
    def ridge_fit_closed(
        ...
        pinv = np.linalg.pinv(xtrain.T @ xtrain + c_lambda * reg)

        weights = pinv @ xtrain.T @ ytrain
    ...
    def ridge_cross_validation(
        ...
        for i in range (kfold):
            ...
            Y_split = np.array_split(y, kfold, axis = 0)
            ...
            if (i == 0):
                X_train = np.concatenate(X_split[1:], axis = 0)
                Y_train = np.concatenate(Y_split[1:], axis = 0)
            ...
            loss_per_fold.append(self.rmse(Y_pred, Y_test))
