...
class LogisticRegression(object):
    ...
    def bias_augment(self, x: np.ndarray) -> np.ndarray:  # [3pts]
        ...
        ones = np.ones((N, 1))  # Create a column of 1's
        x_aug = np.hstack((ones, x))  # Stack the column of 1's to the left of x
        return x_aug
    ...
    def loss(self, y: np.ndarray, h_x: np.ndarray) -> float:  # [3pts]
        ...
        neg = y_inv.T @ np.log(h_x_inv)

        loss = - (pos + neg) / y.shape[0]
    ...
    def accuracy(self, y: np.ndarray, y_hat: np.ndarray) -> float:  # [2pts]
        ...
        N = y.shape[0]
        accuracy = (N - np.count_nonzero(y - y_hat)) / N
        return accuracy
    ...
    def evaluate(
        ...
        y_pred = self.predict_labels(h_x)
