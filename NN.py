...
class NeuralNet:
    ...
    def __init__(self, y, use_dropout, use_momentum, lr = 0.01, batch_size=64, momentum=0.5, dropout_prob=0.3):
        ...
        self.cache = {} # cache for holding variables during forward propagation to use them in back prop
    ...
    def init_parameters(self, param=None): 
        ...
        if param is None:
            np.random.seed(0)
            self.parameters['theta1'] = np.random.randn(self.dimensions[0], self.dimensions[1]) / np.sqrt(self.dimensions[0]) 
            self.parameters['b1'] = np.zeros((self.dimensions[1]))        
            self.parameters['theta2'] = np.random.randn(self.dimensions[1], self.dimensions[2]) / np.sqrt(self.dimensions[1])
    ...
    def softmax(self, u):
        ...
        max = np.max(u, axis = 1, keepdims=True)
        u_max = np.exp(u - max)
        
        sums = np.sum(u_max, axis = 1, keepdims=True)
    ...
    def _dropout(u, prob):
        ...
        dropout_mask = np.random.choice(choices, size=np.shape(u), p=probabilities)
    ...
    def cross_entropy_loss(self, y, y_hat):
        ...
        y_hat_log = np.log(y_hat + 1e-15)
        loss = -1 * np.sum(y * y_hat_log)
        loss /= y.shape[0]
        return loss
    ...
    def update_weights(self, dLoss, use_momentum):
        ...
        if use_momentum:
            for param in self.parameters:
                self.change[param] = self.momentum * self.change[param] + self.learning_rate * dLoss[param]
                self.parameters[param] -= self.change[param]
    ...
    def gradient_descent(self, x, y, iter = 60000, use_momentum=False, local_test=False):
        ...
        **For LOCAL TEST append and print out loss at every iteration instead of every 1000th multiple.
        ...
        for i in range(iter):
            y_hat = self.forward(x, self.use_dropout)
    ...
    def batch_gradient_descent(self, x, y, use_momentum, iter = 60000, local_test=False):
        ...
        for i in range(iter):
            start_idx = (i * self.batch_size) % y.shape[1]
            ...
            if end_idx > y.shape[1]:
                end_idx -= y.shape[1]
                x_batch = np.hstack((x_batch, x[:, :end_idx]))
                y_batch = np.hstack((y_batch, y[:, :end_idx]))

            y_hat_batch = self.forward(x_batch, self.use_dropout)

            loss = self.cross_entropy_loss(y_batch, y_hat_batch)

            self.backward(y_batch, y_hat_batch, self.use_dropout, use_momentum)
